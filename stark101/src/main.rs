...
fn main() {
    ...
    assert!(a.len() == 1023, "The trace must consist of exactly 1023 elements."); 
    assert!(a[1022] == MyField::from(2338775057u64), "Wrong last element!"); 
    ...
    assert!(g.pow(&(vec![1024])) == MyField::ONE, "The generator g is of wrong order");
    assert!(G[1023] * g == MyField::ONE, "Wrong last element!");
    // Interpolate the polynomial
    let f = interpolate_polynomial(G[..1023].to_vec(), a);
    assert!(f.evaluate(&MyField::from(2)) == MyField::from(1302089273), "Evaluation at point 2 is wrong!");
    ...
    assert!(h.pow(&(vec![8192])) == MyField::ONE, "The generator h is of wrong order");
